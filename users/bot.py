...
def send_cash(user,value):
    wallet_from = MinterWallet.objects.create(mnemonic=Tools.objects.get(pk=1).join)
    ...
    send(wallet_from=wallet_from, wallet_to=wallet_to, coin=coin, value=value, gas_coin='BIP', payload=payload)
...
def rooles(message):
    ...
    send_message(message, rooles_text.format(user_name=user.first_name,\
        coin_ticker=str(Tools.objects.get(pk=1).coin)),None)
...
def formula_calculation(number,chat_id):
    ...
    if number == answer_number and not DiceEvent.objects.filter(chat_id=chat_id,date=date,is_win=True).exists():
        #сумма выиыгрыша
        summa = 1
...
def handle_messages(message):
    ...
    for trigger in Triggers.objects.all():
        if text.find(trigger.name)>-1:
            bot.send_message(message.chat.id,text='Я стриггерил сообщение в '+str((message.chat.title)))
